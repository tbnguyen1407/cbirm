...
public class ColorCorrelogram {
    ...
    private static double[][] correlogram;
    ...
    public ColorCorrelogram(int maxDist) {
        maxDistance = maxDist;
        initialize();
    }
    ...
    public static double[][] loadImageIndex(String image) {
        String indexStr = ImageIndexHolder.getInstance().loadImageIndex(ImageIndexHolder.FeatureType.ColorCorrelogram,
                image);
    ...
    public static void saveImageIndex(String image, double[][] index) {
        ImageIndexHolder.getInstance().saveImageIndex(ImageIndexHolder.FeatureType.ColorCorrelogram, image,
                indexToString(index));
    }
    ...
    private static void getNumPixelsInNeighbourhood(int x, int y, int[][] quantPixels, int[] correlogramm) {
        ...
        for (int d = 1; d <= maxDistance; d++) {
            if (d > 1)
                correlogramm[d - 1] += correlogramm[d - 2];
            int color = quantPixels[x][y];

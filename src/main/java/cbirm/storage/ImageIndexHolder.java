...
public class ImageIndexHolder {
    ...
    private static final File fileCorlorHistogram = new File("./indices/index-colorhistogram.dat");
    private static final File fileColorCorrelogram = new File("./indices/index-colorcorrelogram.dat");
    private static final File fileColorCoherenceVector = new File("./indices/index-colorcoherencevector.dat");
    private static final File fileDateTime = new File("./indices/index-datetime.dat");
    private static final File fileEdgeDirection = new File("./indices/index-edgedirection.dat");
    private static final File fileLocation = new File("./indices/index-location.dat");
    private static final File fileSemanticTag = new File("./indices/index-semantictag.dat");
    private static final File fileSimilarTags = new File("./indices/index-similartags.dat");
    ...
    private void saveTable2File(File file, Map<String, String> table) throws IOException {
        file.delete();
        FileSystemOperator.checkAndCreateFile(file);
    ...
    private void saveToTable(FeatureType featureType, String image, String index) {
        ...
        switch (featureType) {
            ...
            case SemanticTag:
                ...
                break;
            // case ImageIcon:
            // tableImageIcon.put(normalizedKey,
            // ImageOperator.createImageIcon(normalizedKey));
            default:
                break;
        }
    }
    ...
    private void initilizeTableSimilarTags() {
        tableSimilarTags.put("people",
                "people actress andy lau army avril lavigne baby blink-182 blond hair boy britney spears bruce lee celebrity cheerleaders cheerleading stunt teams chef child children class classroom cyndi dance eat eating emperor faith hill fisherman girl guards han chae young japanese italian kids korean loewy maozedong marley military model musicians officer patrick pcd police prince smile student teacher tourist undead viking visitor witch woman wong tai sin woods");
        tableSimilarTags.put("animal",
                "animal animals pet atomic kitten bear bird birds carp cat chicken coral cormorant dog dragon duck fish goldfish lamb lick liger lion monkey parrot pet pigeon puppy rhino rhinoceros sharks shepherd_dog spitz squirrel starfish stray swan tiger wolf zoo");
        tableSimilarTags.put("food",
                "food bacon bak kut teh beef beer bread burger cake cheese cheesy crab wings chocolate coconut cookie cooking corn crab cupcake cupcakes  curry dessert dish drink eat eating egg eggs escargot fish fishball fries fruit grape mooncake mushroom otah papaya pasta pastry pork potatoes prawn rice roasted salad sashimi sauce scallop snacks soup spaghetti steamboat strawberry sushi tomyam vitasoy wings");
        tableSimilarTags.put("vehicle",
                "vehicle vehicles airplane bikes boat boats bicycle car cars crane cart machine technology truck train");
        tableSimilarTags.put("landscape",
        ...
        tableSimilarTags.put("party",
                "party band friends birthday new year christmas festive fun holiday naked parade");
        tableSimilarTags.put("sport",
                "sport sports archery target badminton barcelona basketball converse cycling football ball boots players kendo manchester united olympic porto ski clothing skiboards snowboarding soccer ball boots players complex swimming pool team teammate water skiing");
        tableSimilarTags.put("recreation",
